...
function getTasksKurz(course_id){
  ...
  return new Promise((resolve, reject) => {
     var sql = "SELECT task_name, task_description, task_dateEntered, task_deadline, course_name, course_teacherName, course_className, course_code FROM task t JOIN course c ON(t.course_id=c.course_id) JOIN student_course st ON(st.course_id=c.course_id) WHERE st.course_id =?;"
